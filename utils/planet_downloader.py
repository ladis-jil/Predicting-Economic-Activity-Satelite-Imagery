'''
More complex interface than Google's to download images from 
Planet. Unlike Google, Planet allows us to query images at a 
certain time. However, Planet's API is not great and there are 
a variety of issues ranging from a little annoying to fairly 
serious. This interface simplifies the use from a user's 
perspective.
'''
...
class PlanetDownloader:
    def __init__(self, api_key, item_type='PSScene3Band'): # PSScene3Band defines the type of image it returns for a list of 9 or so, available to view on https://developers.planet.com/docs/apis/data/items-assets/. For our purposes 3Band appears to make the most sense.
